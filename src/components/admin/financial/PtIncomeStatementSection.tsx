...
/**
 * TeraLoka — PtIncomeStatementSection (Laba Rugi PT) — collapsible
...
export default function PtIncomeStatementSection({ period, appliedFrom, appliedTo, periodLabel }: Props) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (!json.success || !json.data) { setError(json.error?.message ?? 'Gagal memuat Laba/Rugi'); setData(null); }
  ...
  return (
    <ReportCard
      ...
      summary={!loading && hasRows ? (
        <>
          <span className={cn('text-[13px] font-extrabold tabular-nums', isProfit ? 'text-status-healthy' : 'text-status-critical')}>{formatRp(Math.abs(data!.net_income))}</span>
          <span className={cn(BADGE, isProfit ? 'bg-status-healthy/12 text-status-healthy' : 'bg-status-critical/12 text-status-critical')}>{isProfit ? 'Laba' : 'Rugi'}</span>
        </>
      ) : null}
    >
      {loading && <div className="py-8 px-5 text-center text-[12px] text-text-muted">Memuat laba/rugi…</div>}
      ...
      {!loading && data && !hasRows && (
        <div className="py-10 px-5 text-center text-[12px] text-text-muted">
          <Inbox className="w-7 h-7 mx-auto mb-2 text-text-muted" /> Belum ada pendapatan atau beban pada periode ini.
        </div>
      )}
      {!loading && hasRows && (
        <div className="overflow-x-auto">
          <table className="w-full border-collapse">
            <tbody>
              <tr><td colSpan={2} className="px-[14px] py-2 text-[10px] font-extrabold uppercase tracking-wider text-ads bg-surface-muted/60 border-t border-border">Pendapatan</td></tr>
              {data!.revenue.map((r) => (
              ...
              <tr className="border-t-2 border-ads bg-surface-muted/60">
                <td className="px-[14px] py-3 text-left text-[13px] font-extrabold text-text">{isProfit ? 'Laba Bersih' : 'Rugi Bersih'}</td>
                <td className={cn('px-[14px] py-3 text-right text-[14px] font-extrabold tabular-nums', isProfit ? 'text-status-healthy' : 'text-status-critical')}>{formatRp(Math.abs(data!.net_income))}</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      {!loading && hasRows && (
        <div className="px-[14px] py-2.5 text-[10px] text-text-subtle border-t border-border leading-relaxed">
          Laba/Rugi = Pendapatan − Beban (accrual, dari jurnal PT pada periode terpilih). Pendapatan iklan diakui straight-line (earned), bisa berbeda dari kas masuk. Basis laporan pajak PT.
        </div>
      )}
    </ReportCard>
  );
