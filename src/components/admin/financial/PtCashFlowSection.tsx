...
/**
 * TeraLoka — PtCashFlowSection (Laporan Arus Kas PT) — TAILWIND
 * Sesi Financial PT (2 Jun 2026)
 * ────────────────────────────────────────────────────────────────
 * Arus kas NYATA (direct method) per aktivitas: Operasi / Investasi / Pendanaan.
 * Endpoint: GET /money/revenue/cash-flow?perspective=pt&<periode>
 *
 * Period-aware (ikut filter dashboard) — arus kas lazimnya per-periode.
 * Style & pola konsisten dgn PtIncomeStatementSection.
 */
...
interface CashFlow {
  ...
  investasi:        CFLine[];
  pendanaan:        CFLine[];
  total_operasi:    number;
  total_investasi:  number;
  total_pendanaan:  number;
  net_cash_flow:    number;
}

const formatRpSigned = (n: number) => {
  if (n === 0) return '—';
  const sign = n < 0 ? '−' : '+';
...
export default function PtCashFlowSection({ period, appliedFrom, appliedTo, periodLabel }: Props) {
  const { token } = useAuth();
  ...
  const fetchData = useCallback(async () => {
    ...
    const periodQuery =
      period === 'custom' && appliedFrom && appliedTo
        ...
        : `period=${period}`;
    try {
      const res = await fetch(`${API}/money/revenue/cash-flow?perspective=pt&${periodQuery}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const json = await res.json();
      if (!json.success || !json.data) {
        setError(json.error?.message ?? 'Gagal memuat Arus Kas');
        setData(null);
      } else {
        setData(json.data);
      }
    ...
    <>
      ...
      {rows.length === 0 ? (
        ...
        <tr key={`${title}-${r.entry_type}`} className="border-t border-border">
          ...
          <td className={cn(
            'px-[14px] py-2.5 text-right text-[12px] tabular-nums',
            r.amount < 0 ? 'text-status-critical' : 'text-status-healthy',
          )}>{formatRpSigned(r.amount)}</td>
        </tr>
      ))}
      <tr className="border-t border-border">
        <td className="px-[14px] py-2.5 text-left text-[12px] font-bold text-text-muted">Kas Bersih dari {title}</td>
        <td className={cn(
          'px-[14px] py-2.5 text-right text-[12px] font-bold tabular-nums',
          total < 0 ? 'text-status-critical' : 'text-text',
        )}>{formatRpSigned(total)}</td>
      </tr>
    </>
  );
  ...
  return (
    <div className="bg-surface border border-border rounded-xl overflow-hidden mb-5">
      ...
      <div className="flex items-center justify-between gap-3 px-[18px] py-3.5 border-b border-border flex-wrap">
        <div>
          <p className="text-[14px] font-bold text-text flex items-center gap-1.5"><ArrowLeftRight className="w-4 h-4 text-ads" /> Laporan Arus Kas</p>
          <p className="text-[11px] text-text-muted mt-0.5">
            Aliran kas nyata · PT TeraLoka · {periodLabel}
          </p>
        </div>
        {!loading && data && hasRows && (
          <span className={cn(
            ...
            {isPositive ? 'Kas Naik' : 'Kas Turun'}
          </span>
        )}
      </div>

      {loading && (
        <div className="py-8 px-5 text-center text-[12px] text-text-muted">Memuat arus kas…</div>
      )}
      {!loading && error && (
            ...
            <tbody>
              ...
              <tr className="border-t-2 border-ads bg-surface-muted/60">
                <td className="px-[14px] py-3 text-left text-[13px] font-extrabold text-text">
                  {isPositive ? 'Kenaikan Kas Bersih' : 'Penurunan Kas Bersih'}
                </td>
                <td className={cn(
      ...
      {!loading && data && hasRows && (
        <div className="px-[14px] py-2.5 text-[10px] text-text-subtle border-t border-border leading-relaxed">
          Arus kas = uang nyata masuk/keluar rekening (metode langsung). Berbeda dari Laba/Rugi
          (accrual) — kas masuk dihitung saat diterima, bukan saat diakui. Periode sampai tanggal terpilih.
        </div>
      )}
    </div>
  );
