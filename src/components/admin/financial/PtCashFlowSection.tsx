...
/**
 * TeraLoka — PtCashFlowSection (Arus Kas PT, direct) — collapsible
    ...
    >
      ...
      {!loading && data && !hasRows && (
        <div className="py-10 px-5 text-center text-[12px] text-text-muted">
          <Inbox className="w-7 h-7 mx-auto mb-2 text-text-muted" /> Belum ada arus kas pada periode ini.
        </div>
      )}
      {!loading && hasRows && (
      ...
      {!loading && hasRows && (
        <div className="px-[14px] py-2.5 text-[10px] text-text-subtle border-t border-border leading-relaxed">
          Arus kas = uang nyata masuk/keluar rekening (metode langsung). Berbeda dari Laba/Rugi (accrual) — kas masuk dihitung saat diterima, bukan saat diakui. Periode sampai tanggal terpilih.
        </div>
      )}
    </ReportCard>
  );
