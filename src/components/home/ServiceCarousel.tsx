...
const ALL_SERVICES = [
  ...
  { name: 'Speedboat',  sub: 'BAPASIAR Speed',         href: '#', bg: 'rgba(8,145,178,0.1)',   stroke: '#0891B2', active: false },
  { name: 'Kendaraan',  sub: 'Sewa & Jual',            href: '#', bg: 'rgba(139,92,246,0.1)',  stroke: '#7c3aed', active: false },
  { name: 'Kapal Lokal',sub: 'Overnight Multi-stop',   href: '#', bg: 'rgba(14,116,144,0.1)',  stroke: '#0e7490', active: false },
  { name: 'Feri',       sub: 'BAPASIAR Feri',          href: '#', bg: 'rgba(8,145,178,0.08)',  stroke: '#0e7490', active: false },
  { name: 'Pelni',      sub: 'Info & Jadwal',          href: '#', bg: 'rgba(27,107,74,0.1)',   stroke: '#1B6B4A', active: false },
  { name: 'Jasa',       sub: 'Tukang & Teknisi',       href: '#', bg: 'rgba(236,72,153,0.1)',  stroke: '#db2777', active: false },
  { name: 'PPOB',       sub: 'Bayar Tagihan',          href: '#', bg: 'rgba(99,102,241,0.1)',  stroke: '#6366f1', active: false },
  { name: 'Event Lokal',sub: 'Tiket & Kegiatan',       href: '#', bg: 'rgba(232,150,58,0.1)',  stroke: '#E8963A', active: false },
]
...
const ICONS: Record<string, string> = {
  ...
  'Kendaraan':  'M1 3h15a1 1 0 0 1 1 1v9H1V4z M16 8h4l3 3v5h-7V8Z M5.5 21a2.5 2.5 0 1 0 0-5 2.5 2.5 0 0 0 0 5z M18.5 21a2.5 2.5 0 1 0 0-5 2.5 2.5 0 0 0 0 5z',
  'Kapal Lokal':'M3 18l2-7h14l2 7H3z M12 11V4 M12 4L7 9 M12 4L17 8 M2 21Q12 19 22 21',
  'Feri':       'M1 13h22v6a1 1 0 0 1-1 1H2a1 1 0 0 1-1-1v-6z M4 13V9h16v4 M9 9V6h6v3 M1 19h22',
  'Pelni':      'M2 17l2-6h16l2 6H2z M5 11V7h14v4 M10 7V4 M14 7V4 M1 20Q12 18 23 20',
  ...
  'PPOB':       'M2 3h20v14a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V3z M8 21h8 M12 17v4',
...
export default function ServiceCarousel() {
  ...
  return (
    <div className="relative" style={{ background: '#fff', borderTop: '1px solid var(--border-light)', borderBottom: '1px solid var(--border-light)', padding: '20px 0' }}>
      <div className="absolute top-0 bottom-0 left-0 w-12 z-10 pointer-events-none"
      ...
      >
        {ALL_SERVICES.map((svc) => {
          const iconPath = ICONS[svc.name] || 'M12 2L2 7l10 5 10-5-10-5z'

          const pill = (
            <div className="flex items-center gap-2.5 rounded-full flex-shrink-0 transition-all duration-200 relative"
              style={{
                padding: '10px 18px 10px 12px', whiteSpace: 'nowrap',
                background: 'var(--surface-low)', border: '1.5px solid transparent',
                opacity: svc.active ? 1 : 0.45,
                cursor: svc.active ? 'pointer' : 'default',
              }}
              onMouseEnter={(e) => {
                if (!svc.active) return
                const el = e.currentTarget
                el.style.background = '#fff'
                el.style.borderColor = `${svc.stroke}35`
                el.style.boxShadow = `0 4px 16px ${svc.stroke}18`
            ...
            >
              <div style={{ width: 36, height: 36, borderRadius: 10, background: svc.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                <svg viewBox="0 0 24 24" width={18} height={18} fill="none" stroke={svc.stroke} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
                  {iconPath.split(' M').map((seg, i) => <path key={i} d={i === 0 ? seg : 'M' + seg} />)}
                </svg>
              </div>
              <div>
                <div className="text-[13px] font-bold" style={{ color: 'var(--text)' }}>{svc.name}</div>
                <div className="text-[11px] mt-px" style={{ color: 'var(--text-light)' }}>{svc.sub}</div>
              </div>
              {!svc.active && (
                <span style={{ position: 'absolute', top: -7, right: 8, fontSize: 8, fontWeight: 800, letterSpacing: '0.3px', textTransform: 'uppercase', background: '#F3F4F6', color: '#9CA3AF', padding: '2px 5px', borderRadius: 99, border: '1px solid #E5E7EB' }}>
                  Soon
                </span>
              )}
            </div>
          ...
          return svc.active ? (
            ...
            <div key={svc.name} style={{ flexShrink: 0 }}>{pill}</div>
          )
        })}
      </div>
    </div>
