...
function parseExcerpt(excerpt: string | null | undefined, body?: string): string {
  // Prioritas: excerpt dulu, kalau kosong pakai body
  const candidates = [excerpt, body].filter(Boolean) as string[];
  
  for (const raw of candidates) {
    if (!raw?.trim()) continue;
    
    // Coba parse JSON kalau dimulai dengan {
    if (raw.trim().startsWith('{')) {
      try {
        const parsed = JSON.parse(raw);
        const text = parsed?.content || parsed?.excerpt || parsed?.body || parsed?.text || '';
        if (text) return text.replace(/\n/g, ' ').slice(0, 160).trim() + '...';
      } catch {
        // Coba extract content field manual (untuk JSON yang sedikit rusak)
        const match = raw.match(/"content"\s*:\s*"([\s\S]{20,}?)"\s*[,}]/);
        if (match?.[1]) return match[1].replace(/\n/g, ' ').slice(0, 160).trim() + '...';
      }
    }
    
    // Bukan JSON — pakai langsung (skip kalau dimulai dengan {)
    if (!raw.trim().startsWith('{')) {
      return raw.slice(0, 160).trim() + (raw.length > 160 ? '...' : '');
    }
  }
  return '';
}
