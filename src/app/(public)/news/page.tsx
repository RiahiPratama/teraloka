...
/** Fix excerpt/body yang masih raw JSON dari Groq lama */
  ...
  for (const raw of candidates) {
    if (raw.trim().startsWith('{')) {
      ...
      try {
        ...
        // Regex fallback — ambil nilai "content": "..."
        const m = raw.match(/"content"\s*:\s*"([\s\S]{10,}?)(?:","|\"})/);
        if (m?.[1]) return m[1].replace(/\\n/g, ' ').replace(/\\"/g, '"').trim().slice(0, 180);
      }
      continue; // JSON gagal, coba kandidat berikutnya
    }
    // Teks biasa
...
function AdBanner() {
  return (
    <div className="w-full h-20 flex items-center justify-center rounded-xl my-3"
      style={{ background: '#F9FAFB', border: '1.5px dashed #D1D5DB' }}>
      <div className="text-center">
        <p className="text-xs font-bold text-gray-400 uppercase tracking-wider">IKLAN MITRA TERALOKA</p>
        <p className="text-xs text-gray-300 mt-0.5">728 × 90 — <a href="mailto:ads@teraloka.com" className="text-[#003526]">ads@teraloka.com</a></p>
      </div>
    </div>
  );
}
...
function AdNative() {
  return (
    <div className="flex items-center gap-3 rounded-xl p-4 my-3"
      style={{ background: '#F9FAFB', border: '1.5px dashed #D1D5DB' }}>
      <div className="w-12 h-12 rounded-xl bg-gray-200 flex items-center justify-center text-xl shrink-0">📢</div>
      <div className="flex-1 min-w-0">
        <p className="text-xs font-bold text-gray-400 uppercase tracking-wider">Iklan Mitra TeraLoka</p>
        <p className="text-sm font-semibold text-gray-700 mt-0.5">Pasang iklan di sini</p>
        <p className="text-xs text-gray-400">Jangkau warga Maluku Utara setiap hari</p>
      </div>
      <a href="mailto:ads@teraloka.com"
        className="shrink-0 text-xs font-bold text-white px-4 py-2 rounded-full hover:opacity-90"
...
function AdSidebar({ height = 260, label = '300 × 250' }: { height?: number; label?: string }) {
  return (
    <div className="flex flex-col items-center justify-center gap-3 rounded-xl"
      style={{ height, background: '#F9FAFB', border: '1.5px dashed #D1D5DB' }}>
      <span className="text-3xl">📢</span>
      <div className="text-center px-4">
        <p className="text-xs font-bold text-gray-400 uppercase tracking-wider">Iklan Mitra</p>
        <p className="text-xs text-gray-300 mt-0.5">{label}</p>
      </div>
      <a href="mailto:ads@teraloka.com"
        className="text-xs font-bold text-white px-4 py-1.5 rounded-full hover:opacity-90"
        style={{ background: '#003526' }}>
        Pasang Iklan →
      </a>
    </div>
...
function ShareButtons({ title, slug, className = '' }: { title: string; slug: string; className?: string }) {
  return (
    <div className={`flex items-center gap-2 ${className}`}>
      <button
        onClick={e => { e.preventDefault(); shareWA(title, slug); }}
        className="flex items-center gap-1.5 text-xs font-bold text-green-600 hover:opacity-80 transition-opacity"
      >
      ...
      <button
        onClick={e => { e.preventDefault(); shareFB(slug); }}
        className="flex items-center gap-1.5 text-xs font-bold text-blue-600 hover:opacity-80 transition-opacity"
      >
        <svg className="w-3 h-3" viewBox="0 0 24 24" fill="currentColor">
...
function NewsPageContent() {
  ...
  // Excerpt dari featured yang sudah di-parse
    ...
    <div className="min-h-screen bg-white">
      ...
      {breaking.length > 0 && (
        <div className="max-w-4xl mx-auto px-4 pt-3">
          <div className="flex items-center gap-3 bg-[#003526] text-white rounded-xl px-4 py-2.5 overflow-hidden">
      ...
      <div className="max-w-4xl mx-auto px-4">
        ...
        {!loading && articles.length === 0 && (
          <div className="py-20 text-center">
            ...
            <p className="font-bold text-gray-700 text-lg">Belum ada berita</p>
            ...
            <div className="lg:col-span-8">
              ...
              {featured && (
                <Link href={`/news/${featured.slug}`} className="group block mb-6">
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    {featured.source === 'balapor' && (
                      <span className="text-xs text-[#0891B2] font-semibold">📢 Laporan Warga</span>
                    )}
                  </div>
                  ...
                  {/* Excerpt — selalu pakai parseExcerpt, tidak pakai guard featured.excerpt */}
                  {featuredExcerpt && (
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-gray-400">
                      {featured.author?.name || 'Redaksi'} · {timeAgo(featured.published_at)}
                    </span>
                      ...
                      <Link href={`/news/${article.slug}`}
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-1.5 mb-1">
                            ...
                            {article.category && (
                              <span className="text-xs font-bold text-[#003526] uppercase tracking-wide">
                                {article.category}
                              </span>
                            )}
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            {/* Share buttons — muncul saat hover */}
              ...
              {hasMore && (
                <button onClick={() => fetchArticles(false)} disabled={loadingMore}
                  className="w-full mt-6 py-3 border border-gray-200 rounded-xl text-sm font-semibold text-gray-600 hover:bg-gray-50 disabled:opacity-50 transition-colors">
            ...
            <div className="hidden lg:block lg:col-span-4">
              <div className="sticky top-36 space-y-5">
                ...
                <div className="bg-gray-50 rounded-2xl p-4">
                  ...
                  {articles.slice(0, 5).map((a, i) => (
                    <Link key={a.id} href={`/news/${a.slug}`}
                      ...
                      <p className="text-xs font-semibold text-gray-700 leading-snug group-hover:text-[#003526] transition-colors line-clamp-3">
                        {a.title}
                      </p>
                    </Link>
                  ))}
                ...
                <div className="bg-[#003526] rounded-2xl p-5 text-center">
                  ...
                  <p className="text-xs mb-3" style={{ color: '#95d3ba' }}>Laporkan via BALAPOR. Identitasmu terlindungi.</p>
                  <Link href="/reports"
                    className="block text-center bg-white text-xs font-black px-4 py-2 rounded-xl"
                    style={{ color: '#003526' }}>
                    Lapor Sekarang →
                  </Link>
                </div>
