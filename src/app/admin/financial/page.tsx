    ...
    <div style={{ fontFamily: "'Outfit', system-ui, sans-serif", color: t.textPrimary }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <p style={{ margin: '4px 0 0', fontSize: 13, color: t.textMuted }}>Ringkasan pendapatan dari semua layanan TeraLoka</p>
        </div>
      ...
      {loading ? (
        <div style={{ textAlign: 'center', padding: 80, color: t.textMuted }}>Memuat data keuangan...</div>
      ) : (
        <>
          {/* KPI Cards */}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 12, marginBottom: 20 }}>
            <div style={{ background: 'linear-gradient(135deg, #1B6B4A, #0F4A34)', border: '1px solid #1B6B4A', borderRadius: 14, padding: '18px 20px' }}>
            ...
            {Object.entries(SOURCE_CONFIG).slice(0, 4).map(([key, cfg]) => (
              <div key={key} style={{ background: t.card, border: `1px solid ${t.cardBorder}`, borderRadius: 14, padding: '16px 18px' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                  <p style={{ margin: 0, fontSize: 11, color: t.textMuted, fontWeight: 600 }}>{cfg.label}</p>
            ...
            <div style={{ background: t.card, border: `1px solid ${t.cardBorder}`, borderRadius: 14, padding: '16px 18px' }}>
              ...
              <ResponsiveContainer width="100%" height={180}>
                <LineChart data={trendData}>
                  ...
                  <Tooltip contentStyle={{ background: t.card, border: `1px solid ${t.cardBorder}`, borderRadius: 8, fontSize: 11, color: t.textPrimary }}
            ...
            <div style={{ background: t.card, border: `1px solid ${t.cardBorder}`, borderRadius: 14, padding: '16px 18px' }}>
              ...
              {donutData.length === 0 ? (
                <div style={{ height: 180, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 8 }}>
                  <p style={{ color: t.textDim, fontSize: 13 }}>Belum ada data revenue</p>
                    ...
                    {donutData.map((d, i) => {
                      ...
                      return (
                        <div key={i}>
                          ...
                          <div style={{ marginLeft: 14, height: 3, background: t.cardInner, borderRadius: 2 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14, marginBottom: 20 }}>
                ...
                {FUNNEL_DATA.map((f, i) => (
                  <div key={i}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                      <div>
                        <p style={{ margin: 0, fontSize: 12, fontWeight: 600, color: t.textPrimary }}>{f.label}</p>
                        <p style={{ margin: 0, fontSize: 10, color: t.textMuted }}>{f.desc}</p>
                      </div>
                      <span style={{ fontSize: 12, fontWeight: 700, color: f.color }}>{f.pct}%</span>
                    </div>
                    <div style={{ height: 4, background: t.cardInner, borderRadius: 2 }}>
                ...
                <thead>
                  <tr style={{ borderBottom: `1px solid ${t.cardBorder}` }}>
                    {['Layanan', 'Revenue', '%'].map(h => (
                      <th key={h} style={{ padding: '4px 0', textAlign: h === 'Layanan' ? 'left' : 'right', fontSize: 10, color: t.textMuted, fontWeight: 600 }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {Object.entries(SOURCE_CONFIG).map(([key, cfg]) => {
                    ...
                    return (
                      <tr key={key} style={{ borderBottom: `1px solid ${t.deepBg}` }}>
                        <td style={{ padding: '8px 0' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                            <span>{cfg.icon}</span>
                            <span style={{ color: t.textPrimary, fontWeight: 500 }}>{cfg.label}</span>
                          </div>
                        </td>
                        <td style={{ padding: '8px 0', textAlign: 'right', color: cfg.color, fontWeight: 700 }}>{formatRp(amt)}</td>
                        <td style={{ padding: '8px 0', textAlign: 'right', color: t.textMuted }}>{pct}%</td>
                      </tr>
                    );
                  })}
                  <tr style={{ borderTop: `1px solid ${t.cardBorder}` }}>
                    <td style={{ padding: '8px 0', fontWeight: 700, color: t.textPrimary }}>Total</td>
                    <td style={{ padding: '8px 0', textAlign: 'right', fontWeight: 800, color: t.codeText }}>{formatRp(thisTotal)}</td>
                    <td style={{ padding: '8px 0', textAlign: 'right', color: t.codeText, fontWeight: 700 }}>100%</td>
                  </tr>
                </tbody>
              </table>
            </div>
            ...
            <div style={{ background: t.card, border: `1px solid ${t.cardBorder}`, borderRadius: 14, padding: '16px 18px' }}>
              ...
              {topList.length === 0 ? (
                <p style={{ color: t.textDim, fontSize: 13 }}>Belum ada data</p>
                ...
                return (
                  <div key={key} style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
                    <span style={{ fontSize: 16, fontWeight: 800, color: t.textDim, width: 16, flexShrink: 0 }}>{i + 1}</span>
                    <span style={{ fontSize: 16, flexShrink: 0 }}>{cfg.icon}</span>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
                        <span style={{ fontSize: 12, fontWeight: 600, color: t.textPrimary }}>{cfg.label}</span>
                        <span style={{ fontSize: 12, fontWeight: 700, color: cfg.color }}>{formatRp(val.total)}</span>
                      </div>
                      <div style={{ height: 3, background: t.cardInner, borderRadius: 2 }}>
                        <div style={{ width: `${pct}%`, height: '100%', background: cfg.color, borderRadius: 2 }} />
                      </div>
                      <p style={{ margin: '2px 0 0', fontSize: 10, color: t.textMuted }}>{val.count} transaksi · {pct}%</p>
                    </div>
                  </div>
                );
              })}
          ...
          <div style={{ marginBottom: 20, padding: '12px 16px', borderRadius: 10,
            ...
            {thisTotal >= 10000000 ? (
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8 }}>
                ...
                <div style={{ height: 6, background: t.cardInner, borderRadius: 3, flex: 1, minWidth: 120, maxWidth: 200 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: showForm ? '1fr 360px' : '1fr', gap: 14 }}>
            ...
            <div style={{ background: t.card, border: `1px solid ${t.cardBorder}`, borderRadius: 14, overflow: 'hidden' }}>
              <div style={{ padding: '14px 18px', borderBottom: `1px solid ${t.cardBorder}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <span style={{ fontSize: 12, color: t.textMuted }}>Total {records.length} transaksi</span>
              </div>
              {records.length === 0 ? (
                ...
                return (
                  <div key={r.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 18px', borderBottom: i < records.length - 1 ? `1px solid ${t.cardBorder}` : 'none' }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <p style={{ margin: 0, fontSize: 13, fontWeight: 600, color: t.textPrimary, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.description}</p>
                    ...
                    <div style={{ textAlign: 'right', flexShrink: 0 }}>
                      <p style={{ margin: 0, fontSize: 14, fontWeight: 800, color: t.codeText }}>{formatRp(r.amount)}</p>
            ...
            {showForm && (
              <div style={{ background: t.card, border: `1px solid ${t.cardBorder}`, borderRadius: 14, padding: '18px', height: 'fit-content', position: 'sticky', top: 80 }}>
                ...
                <div style={{ marginBottom: 12 }}>
                  <label style={{ fontSize: 11, color: t.textMuted, fontWeight: 600, display: 'block', marginBottom: 4 }}>Sumber Revenue</label>
                  <select value={form.type} onChange={e => setForm(p => ({ ...p, type: e.target.value }))}
                    style={{ width: '100%', padding: '8px 12px', background: t.inputBg, border: `1px solid ${t.inputBorder}`, borderRadius: 8, color: t.textPrimary, fontSize: 13, outline: 'none', boxSizing: 'border-box' as const }}>
                    {Object.entries(SOURCE_CONFIG).map(([k, v]) => <option key={k} value={k}>{v.icon} {v.label}</option>)}
                  </select>
                </div>

                <div style={{ marginBottom: 12 }}>
                  <label style={{ fontSize: 11, color: t.textMuted, fontWeight: 600, display: 'block', marginBottom: 4 }}>Nominal (Rp) *</label>
                ...
                <div style={{ marginBottom: 16 }}>
                  <label style={{ fontSize: 11, color: t.textMuted, fontWeight: 600, display: 'block', marginBottom: 4 }}>Deskripsi *</label>
                  <input value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))}
                    placeholder='Iklan "Toko Rempah" paket Starter'
                    style={{ width: '100%', padding: '8px 12px', background: t.inputBg, border: `1px solid ${t.inputBorder}`, borderRadius: 8, color: t.textPrimary, fontSize: 13, outline: 'none', boxSizing: 'border-box' as const }} />
                </div>

                <div style={{ background: t.deepBg, borderRadius: 8, padding: '10px 12px', marginBottom: 16 }}>
