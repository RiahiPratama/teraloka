...
export default function BakabarOverviewPage() {
  const { token } = useAuth();
  ...
  const Card = ({ label, value, sub, color = '#1B6B4A' }: { label: string; value: string | number; sub?: string; color?: string }) => (
    <div style={{
      background: t.sidebar, borderRadius: 14, border: `1px solid ${t.sidebarBorder}`,
    ...
    <div style={{ maxWidth: 1100, margin: '0 auto', fontFamily: "'Outfit', system-ui" }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        <div>
          ...
          <p style={{ color: t.textDim, fontSize: 13, marginTop: 3 }}>BAKABAR — Overview hari ini</p>
        </div>
        <Link href="/admin/bakabar/hub" style={{
          padding: '8px 16px', borderRadius: 10, background: '#1B6B4A',
      ...
      <div style={{
        ...
        borderBottom: `1px solid ${t.sidebarBorder}`, paddingBottom: 0,
      }}>
        {['Overview', 'Newsroom Analytics', 'Distribution Metrics'].map((tab, i) => (
          <div key={tab} style={{
            padding: '8px 16px', fontSize: 13, fontWeight: i === 0 ? 700 : 500,
            color: i === 0 ? '#1B6B4A' : t.textDim, cursor: 'pointer',
            borderBottom: i === 0 ? '2px solid #1B6B4A' : '2px solid transparent',
            marginBottom: -1,
          }}>
            {tab}
          </div>
      ...
      {loading ? (
        ...
        <>
          ...
          <div style={{ display: 'flex', gap: 12, marginBottom: 20 }}>
            <Card label="Artikel Terbit Hari Ini" value={published.length} sub="artikel published" color="#1B6B4A" />
            <Card label="Total Views Hari Ini" value={`${(totalViews / 1000).toFixed(1)}K`} sub="dari semua artikel" color="#0891B2" />
            <Card label="Artikel Dipublish > 3 Jam" value={staleDrafts.length} sub="draft perlu diperhatikan" color={staleDrafts.length > 0 ? '#F59E0B' : '#10B981'} />
          </div>

          {/* Middle row: Trending + Perlu Perhatian */}
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
            ...
            <div style={{ background: t.sidebar, borderRadius: 14, border: `1px solid ${t.sidebarBorder}`, padding: '18px 20px' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
                ...
                <Link href="/admin/bakabar/trending" style={{ fontSize: 11, color: '#1B6B4A', textDecoration: 'none', fontWeight: 600 }}>
                  See All →
                </Link>
              </div>
                  ...
                  <div key={a.id} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <p style={{ fontSize: 11, color: t.textDim }}>
                        +{a.view_count.toLocaleString('id-ID')} views (+{a.viral_score} score)
                      </p>
                    </div>
                    <Link href={`/news/${a.slug}`} target="_blank" style={{
            ...
            {/* Perlu Perhatian */}
            <div style={{
              background: staleDrafts.length > 0 ? 'rgba(245,158,11,0.04)' : t.sidebar,
              borderRadius: 14,
              border: `1px solid ${staleDrafts.length > 0 ? '#FDE68A' : t.sidebarBorder}`,
              padding: '18px 20px',
            }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
                <span style={{ fontSize: 13, fontWeight: 800, color: t.textPrimary }}>⚠️ Perlu Perhatian</span>
                <Link href="/admin/bakabar/hub?status=draft" style={{ fontSize: 11, color: '#F59E0B', textDecoration: 'none', fontWeight: 600 }}>
                  Review →
                </Link>
              </div>
              {staleDrafts.length === 0 ? (
                ...
                <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: 8 }}>
                  <li style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: '#92400E' }}>
                    ...
                    {staleDrafts.length} artikel belum dipublish {'>'} 3 jam
                  </li>
                  {drafts.length > 0 && (
                    <li style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: '#92400E' }}>
          ...
          {/* Editorial Dashboard */}
          <div style={{ background: t.sidebar, borderRadius: 14, border: `1px solid ${t.sidebarBorder}`, padding: '20px' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
              <span style={{ fontSize: 15, fontWeight: 800, color: t.textPrimary }}>Editorial Dashboard</span>
              <Link href="/admin/bakabar/hub" style={{ fontSize: 11, color: '#1B6B4A', textDecoration: 'none', fontWeight: 600 }}>
                Buka Hub →
              </Link>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
              ...
              <div>
                ...
                <div style={{
                  ...
                  padding: '0 0 6px', borderBottom: `1px solid ${t.sidebarBorder}`,
                  marginBottom: 6,
                }}>
                  <span>Artikel</span>
                ...
                {trending.map((a, i) => (
                  <div key={a.id} style={{
                    ...
                    padding: '7px 0', borderBottom: `1px solid ${t.sidebarBorder}`,
                    alignItems: 'center',
                  }}>
                    <div style={{ minWidth: 0, paddingRight: 8 }}>
                      <div style={{
                        ...
                        <span style={{
                          ...
                          borderRadius: '50%', background: i < 2 ? '#F97316' : '#E5E7EB',
                      ...
                      <div style={{ fontSize: 10, color: t.textDim, paddingLeft: 22 }}>
                        {a.category} · {timeAgo(a.published_at || a.created_at)}
                      </div>
                    </div>
                    <div style={{ textAlign: 'right', fontSize: 13, fontWeight: 800, color: a.viral_score > 100 ? '#F97316' : t.textPrimary }}>
                      {a.viral_score}
                    </div>
                    <div style={{ textAlign: 'right', fontSize: 12, fontWeight: 600, color: t.textMuted }}>
              ...
              <div>
                ...
                <div style={{
                  display: 'grid',
                  gridTemplateColumns: '80px 1fr 50px 30px',
                  fontSize: 9, fontWeight: 800, color: t.textDim,
                  textTransform: 'uppercase', letterSpacing: '0.06em',
                  padding: '0 0 6px', borderBottom: `1px solid ${t.sidebarBorder}`,
                  marginBottom: 6,
                ...
                {recent.map(a => (
                  <div key={a.id} style={{
                    display: 'grid', gridTemplateColumns: '80px 1fr 50px 30px',
                    padding: '7px 0', borderBottom: `1px solid ${t.sidebarBorder}`,
                    alignItems: 'center',
                  }}>
                    <span style={{
                      ...
                      color: '#059669', display: 'inline-block', width: 'fit-content',
                    }}>Published</span>
                    <div style={{ minWidth: 0, paddingLeft: 8, paddingRight: 8 }}>
                      <p style={{ fontSize: 11, color: t.textDim, marginBottom: 1 }}>
                        {timeAgo(a.published_at || a.created_at)}
                      </p>
                      <p style={{ fontSize: 11, fontWeight: 600, color: t.textPrimary, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        {a.title}
                      </p>
                    </div>
                    <div style={{ textAlign: 'right', fontSize: 12, fontWeight: 700, color: t.textMuted }}>
                      {a.view_count >= 1000 ? `${(a.view_count / 1000).toFixed(1)}k` : a.view_count}
                    </div>
