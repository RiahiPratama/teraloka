...
const LISTING_TYPES = [
  { key: 'kos', label: 'BAKOS', desc: 'Kos-kosan & kontrakan', icon: '🏠' },
...
export default function NewListingPage() {
  ...
  // Step 2 — Detail per tipe
  const [price, setPrice] = useState('');
  const [transactionType, setTransactionType] = useState('');
  const [pricePeriod, setPricePeriod] = useState('bulan');
      ...
      <div className="flex min-h-[60vh] items-center justify-center px-4">
        <div className="text-center max-w-sm">
          ...
          <p className="mt-2 text-sm text-gray-500 leading-relaxed">
            Listing kamu sudah masuk sebagai draft. Tim TeraLoka akan mengaktifkan dalam 1×24 jam.
          </p>
          <div className="mt-5 flex gap-2">
            ...
            <button onClick={() => {
              setSubmitted(false); setStep(0); setType('');
              setTitle(''); setDescription(''); setCity(''); setAddress(''); setPhone('');
              setPrice(''); setTransactionType(''); setFacilities([]);
            }}
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload: Record<string, any> = {
        ...
        price_period: type === 'kos' ? pricePeriod : null,
      };

      if (type === 'kos') {
        payload.kos_type = kosType;
        payload.facilities = facilities;
        payload.room_available = Number(roomAvailable);
      } else if (type === 'properti') {
        payload.property_type = propertyType;
        payload.land_area_m2 = landArea ? Number(landArea) : null;
        payload.building_area_m2 = buildingArea ? Number(buildingArea) : null;
      } else if (type === 'kendaraan') {
        payload.vehicle_type = vehicleType;
        payload.vehicle_brand = vehicleBrand;
        payload.vehicle_year = vehicleYear ? Number(vehicleYear) : null;
        payload.vehicle_condition = vehicleCondition;
      } else if (type === 'jasa') {
      ...
      if (res.ok) {
        ...
        setError(data.error?.message ?? 'Gagal menambah listing.');
      }
    } catch {
  ...
  const canNext = [
    ...
    type === 'properti' ? (!!propertyType && !!transactionType && !!price) :
    type === 'kendaraan' ? (!!vehicleType && !!vehicleBrand && !!transactionType && !!price) :
    type === 'jasa' ? !!serviceCategory : false,
  ][step];

  const STEPS = ['Tipe Listing', 'Info Dasar', 'Detail'];

  return (
    <div className="mx-auto max-w-lg px-4 py-6">
      <div className="mb-5">
        <h1 className="text-xl font-bold text-[#1B6B4A]">Tambah Listing</h1>
        <p className="text-sm text-gray-500">Portal Owner TeraLoka</p>
      </div>
      ...
      <div className="mb-6 flex items-center gap-2">
        {STEPS.map((s, i) => (
          <div key={i} className="flex items-center gap-2">
            ...
            {i < STEPS.length - 1 && (
              <div className={`h-0.5 w-10 ${i < step ? 'bg-[#1B6B4A]' : 'bg-gray-200'}`} />
            )}
          </div>
        ))}
          ...
          {LISTING_TYPES.map(t => (
            <button key={t.key} onClick={() => setType(t.key)}
              className={`rounded-2xl border-2 p-4 text-left transition-all ${
                type === t.key ? 'border-[#1B6B4A] bg-green-50' : 'border-gray-200 bg-white hover:border-gray-300'
              }`}>
        ...
        <div className="space-y-4">
          <div>
            ...
            <input type="text" value={title} onChange={e => setTitle(e.target.value)}
              placeholder={
                type === 'kos' ? 'Contoh: Kos Putri Akehuda dekat Kampus UNKHAIR' :
                type === 'properti' ? 'Contoh: Rumah 3 Kamar di BTN Ternate' :
                type === 'kendaraan' ? 'Contoh: Honda Beat 2022 Mulus' :
                'Contoh: Tukang Listrik Berpengalaman Ternate'
              }
          ...
          <div>
            ...
            <textarea value={description} onChange={e => setDescription(e.target.value)}
              placeholder="Ceritakan lebih detail tentang listing kamu..."
              rows={4}
              className="mt-1.5 w-full rounded-xl border border-gray-200 px-3 py-2.5 text-sm outline-none focus:border-[#1B6B4A]"
            />
          </div>
              ...
              {CITIES.map(c => (
                <button key={c} onClick={() => setCity(c)}
                  className={`rounded-lg px-3 py-1.5 text-xs font-medium transition-colors ${
                    city === c ? 'bg-[#1B6B4A] text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                  }`}>
                  {c}
                </button>
              ))}
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-gray-400">Nomor ini tidak ditampilkan langsung ke publik — dilindungi sistem WA relay</p>
          </div>
        </div>
      )}
        ...
        <div className="space-y-4">
            ...
            <>
                  ...
                  {['putra', 'putri', 'campur'].map(k => (
                    <button key={k} onClick={() => setKosType(k)}
                      className={`flex-1 rounded-xl py-2.5 text-xs font-medium capitalize transition-colors ${
                        kosType === k ? 'bg-[#1B6B4A] text-white' : 'bg-gray-100 text-gray-600'
                      }`}>
              ...
              <div>
                ...
                <div className="relative mt-1.5">
                  ...
                  <input type="text" value={price} onChange={e => setPrice(formatRupiah(e.target.value))}
                    placeholder="800.000"
              ...
              <div>
                ...
                <input type="number" value={roomAvailable} onChange={e => setRoomAvailable(e.target.value)}
                  min="1" max="100"
                  className="mt-1.5 w-full rounded-xl border border-gray-200 px-3 py-2.5 text-sm outline-none focus:border-[#1B6B4A]"
                />
              </div>
                  ...
                  {KOS_FACILITIES.map(f => (
                    <button key={f} onClick={() => toggleFacility(f)}
                      className={`rounded-lg px-2.5 py-1.5 text-xs font-medium transition-colors ${
                        facilities.includes(f) ? 'bg-[#1B6B4A] text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                      }`}>
                      {f}
                    </button>
                  ))}
            ...
            <>
                ...
                <div className="mt-1.5 flex flex-wrap gap-2">
                  {['Rumah', 'Apartemen', 'Ruko', 'Tanah', 'Kos', 'Gudang'].map(p => (
                    <button key={p} onClick={() => setPropertyType(p)}
                      className={`rounded-lg px-3 py-1.5 text-xs font-medium transition-colors ${
                        propertyType === p ? 'bg-[#1B6B4A] text-white' : 'bg-gray-100 text-gray-600'
                      }`}>
                      {p}
                    </button>
                  ))}
                ...
                <div className="mt-1.5 flex gap-2">
                  {['jual', 'sewa'].map(t => (
                    <button key={t} onClick={() => setTransactionType(t)}
                      className={`flex-1 rounded-xl py-2.5 text-xs font-medium capitalize transition-colors ${
                        transactionType === t ? 'bg-[#1B6B4A] text-white' : 'bg-gray-100 text-gray-600'
                      }`}>
              ...
              <div>
                <label className="text-sm font-medium text-gray-700">
                  Harga {transactionType === 'sewa' ? '/ Tahun' : ''}
                </label>
                <div className="relative mt-1.5">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm text-gray-400">Rp</span>
                  <input type="text" value={price} onChange={e => setPrice(formatRupiah(e.target.value))}
          ...
          {type === 'kendaraan' && (
            <>
                ...
                <div className="mt-1.5 flex gap-2">
                  {['motor', 'mobil', 'pickup', 'truk'].map(v => (
                    <button key={v} onClick={() => setVehicleType(v)}
                      className={`flex-1 rounded-xl py-2.5 text-xs font-medium capitalize transition-colors ${
                        vehicleType === v ? 'bg-[#1B6B4A] text-white' : 'bg-gray-100 text-gray-600'
                      }`}>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input type="number" value={vehicleYear} onChange={e => setVehicleYear(e.target.value)}
                    placeholder="2022" min="1990" max={new Date().getFullYear()}
                    className="mt-1.5 w-full rounded-xl border border-gray-200 px-3 py-2.5 text-sm outline-none focus:border-[#1B6B4A]"
                  />
                </div>
              </div>
                ...
                <div className="mt-1.5 flex gap-2">
                  {['baru', 'bekas'].map(c => (
                    <button key={c} onClick={() => setVehicleCondition(c)}
                      className={`flex-1 rounded-xl py-2.5 text-xs font-medium capitalize transition-colors ${
                        vehicleCondition === c ? 'bg-[#1B6B4A] text-white' : 'bg-gray-100 text-gray-600'
                      }`}>
                      {c === 'baru' ? '✨ Baru' : '🔧 Bekas'}
                    </button>
                  ))}
              ...
              <div>
                <label className="text-sm font-medium text-gray-700">
                  Harga {transactionType === 'sewa' ? '/ Hari' : ''}
                </label>
                <div className="relative mt-1.5">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm text-gray-400">Rp</span>
                  <input type="text" value={price} onChange={e => setPrice(formatRupiah(e.target.value))}
            ...
            <>
                  ...
                  {SERVICE_CATEGORIES.map(s => (
                    <button key={s} onClick={() => setServiceCategory(s)}
                      className={`rounded-xl px-3 py-2.5 text-left text-xs font-medium transition-colors ${
                        serviceCategory === s ? 'bg-[#1B6B4A] text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                      }`}>
                      {s}
                    </button>
                  ))}
              ...
              <div>
                ...
                <input type="text" value={serviceArea} onChange={e => setServiceArea(e.target.value)}
                  placeholder="Contoh: Ternate Selatan, Ternate Tengah"
                  className="mt-1.5 w-full rounded-xl border border-gray-200 px-3 py-2.5 text-sm outline-none focus:border-[#1B6B4A]"
                />
              </div>
              ...
              <div>
                <label className="text-sm font-medium text-gray-700">
                  Tarif <span className="text-gray-400">(opsional)</span>
                </label>
                <div className="relative mt-1.5">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm text-gray-400">Rp</span>
                  <input type="text" value={price} onChange={e => setPrice(formatRupiah(e.target.value))}
