    ...
    try {
      ...
      if (res.ok && json.success) {
        if (type === 'delete') {
          ...
          router.replace('/owner/funding/campaigns');
          return;
        }
    ...
    return (
      <div className="flex min-h-[60vh] items-center justify-center px-4">
        <div className="text-center max-w-sm">
          ...
          <button
            onClick={() => router.push(`/login?redirect=/owner/funding/campaigns/${campaignId}`)}
...
function ActionButtons({
  ...
  const editHref = `/owner/funding/campaigns/${campaign.id}/edit`;
...
function DonorsSection({
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
      ...
      {hasMore ? (
        <Link
          href={`/owner/funding/campaigns/${campaignId}/donations`}
