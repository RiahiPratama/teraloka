...
function MyCampaignsContent() {
  ...
  function handleTabChange(tab: StatusTab) {
    ...
    router.replace(`/owner/funding/campaigns${params.toString() ? '?' + params.toString() : ''}`, { scroll: false });
  }
...
function CampaignCard({
  ...
  const detailHref = campaign.status === 'draft'
    ? `/owner/funding/campaigns/${campaign.id}/edit`
    : `/owner/funding/campaigns/${campaign.id}`;
    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
      ...
      {campaign.status === 'draft' && (
        <div className="flex items-center gap-2 px-3 pb-3 pt-0">
          <Link
            href={`/owner/funding/campaigns/${campaign.id}/edit`}
