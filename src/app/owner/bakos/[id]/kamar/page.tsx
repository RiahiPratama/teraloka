...
// ════════════════════════════════════════════════════════════════
// BAKOS Owner — Kelola Kamar
// PATH: src/app/owner/bakos/[id]/kamar/page.tsx
// PENANDA: L5-FE-OWNER-KAMAR
// ────────────────────────────────────────────────────────────────
// List:   GET    /bakos/owner/listings/:id/rooms (owner-scoped)
// Tambah: POST   /bakos/owner/listings/:id/rooms (cap kamar di-enforce → 403)
// Edit:   PUT    /bakos/owner/listings/:id/rooms/:roomId
// Hapus:  DELETE /bakos/owner/listings/:id/rooms/:roomId
// 🛡️ Pesan 403 (ROOM_QUOTA) ditampilkan apa adanya dari backend.
// ════════════════════════════════════════════════════════════════
...
export default function OwnerKamarPage() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const data = await api.get<Room[]>(`/bakos/owner/listings/${id}/rooms`);
      setRooms(data ?? []);
    } catch (e) {
  ...
  const inputCls = 'mt-1 w-full rounded-xl border border-gray-200 px-3 py-2 text-sm outline-none focus:border-[#854F0B]';
  ...
  return (
    <div className="min-h-screen pb-20" style={{ background: BAKOS_TOKENS.pageBg }}>
      <div className="max-w-lg mx-auto px-4 pt-5">
        <button onClick={() => router.push(`/owner/bakos/${id}`)} className="flex items-center gap-1 text-xs mb-3" style={{ color: BAKOS_TOKENS.textSecondary }}>
          <ChevronLeft size={14} /> Kelola Kos
        </button>

        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-lg font-bold" style={{ color: BAKOS_TOKENS.textPrimary }}>Kamar</h1>
            <p className="text-xs" style={{ color: BAKOS_TOKENS.textSecondary }}>{rooms.length} tipe · {totalRooms} kamar total</p>
          </div>
          {editingId === null && (
            <button onClick={openNew} className="flex items-center gap-1.5 rounded-xl px-3 py-2 text-xs font-semibold text-white" style={{ background: BRAND }}>
        ...
        {error && editingId === null && (
          <div className="rounded-xl p-3 flex items-start gap-2 mb-3" style={{ background: '#FDECEC' }}><AlertCircle size={16} className="text-red-600 shrink-0 mt-0.5" /><p className="text-xs text-red-800">{error}</p></div>
        )}

        {/* Editor */}
        {editingId !== null && (
          <div className="rounded-2xl border p-4 mb-4 space-y-3" style={{ background: BAKOS_TOKENS.surface, borderColor: BAKOS_TOKENS.border }}>
            <div className="flex items-center justify-between">
              <p className="text-sm font-semibold" style={{ color: BAKOS_TOKENS.textPrimary }}>{editingId === 'new' ? 'Tipe Kamar Baru' : 'Edit Tipe Kamar'}</p>
              <button onClick={close}><X size={18} className="text-gray-400" /></button>
            </div>
            <div>
              <label className="text-xs font-medium text-gray-700">Nama Tipe</label>
              <input value={draft.room_type} onChange={e => setDraft({ ...draft, room_type: e.target.value })} placeholder="Standar / AC / VIP" className={inputCls} />
            </div>
            <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input value={draft.price} onChange={e => setDraft({ ...draft, price: fmt(e.target.value) })} placeholder="500000" className={inputCls} />
              </div>
              <div>
                <label className="text-xs font-medium text-gray-700">Per</label>
                <div className="mt-1 flex gap-1">{['bulan', 'malam', 'hari'].map(p => (
                  <button key={p} onClick={() => setDraft({ ...draft, price_period: p })} className="flex-1 rounded-xl py-2 text-xs font-medium capitalize" style={chip(draft.price_period === p)}>{p}</button>
                ))}</div>
              </div>
            </div>
            <div className="grid grid-cols-3 gap-2">
              <div><label className="text-xs font-medium text-gray-700">Total</label><input type="number" min="1" value={draft.total_rooms} onChange={e => setDraft({ ...draft, total_rooms: e.target.value, available_rooms: e.target.value })} className={inputCls} /></div>
              <div><label className="text-xs font-medium text-gray-700">Tersedia</label><input type="number" min="0" value={draft.available_rooms} onChange={e => setDraft({ ...draft, available_rooms: e.target.value })} className={inputCls} /></div>
              <div><label className="text-xs font-medium text-gray-700">m²</label><input type="number" min="1" value={draft.size_m2} onChange={e => setDraft({ ...draft, size_m2: e.target.value })} className={inputCls} /></div>
            </div>
            <div>
              <label className="text-xs font-medium text-gray-700">Fasilitas Kamar</label>
              <div className="flex flex-wrap gap-2 mt-1.5">{ROOM_FACILITIES.map(f => (
                <button key={f} onClick={() => setDraft({ ...draft, facilities: draft.facilities.includes(f) ? draft.facilities.filter(x => x !== f) : [...draft.facilities, f] })} className="rounded-lg px-2.5 py-1.5 text-xs font-medium" style={chip(draft.facilities.includes(f))}>{f}</button>
              ))}</div>
            </div>
            <ImageUpload bucket="listings" label="Foto Kamar" onUpload={(urls: string[]) => setDraft({ ...draft, photos: urls })} existingUrls={draft.photos} maxFiles={5} />
            ...
            <div className="flex gap-2">
              ...
              <button onClick={close} className="rounded-xl px-4 py-2.5 text-sm font-medium border border-gray-200 text-gray-600">Batal</button>
            </div>
          </div>
        )}
        ...
        {editingId === null && (
          rooms.length === 0 ? (
            ...
            <div className="space-y-2">
              {rooms.map(r => (
                <div key={r.id} className="rounded-2xl border p-3 flex items-start gap-3" style={{ background: BAKOS_TOKENS.surface, borderColor: BAKOS_TOKENS.border }}>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold truncate" style={{ color: BAKOS_TOKENS.textPrimary }}>{r.room_type}</p>
                    <p className="text-xs mt-0.5" style={{ color: BRAND }}>{formatRp(r.price)}<span style={{ color: BAKOS_TOKENS.textSecondary }}>/{r.price_period}</span></p>
